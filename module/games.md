...
> 里面的所有游戏都需要使用金币作为赌注！ 注：同一时间群内只能有一场对决
...
## 个人账户
    ...
    .我的资料  # 查看个人账户详细资料
    .转账 <金额> <at>  # 给at的用户发金币
    .送道具 <道具名> [道具数量](选填；默认为1) <at>  # 给at的用户送指定数量的道具。可以送路灯挂件牌，道具名：路灯挂件标记。
...
1. 当群内的金币数量总和到达20000时 [群主，管理员，超管] 可以使用 `.市场注册` 把此群号注册到市场。
...
`.市场注册 <公司名称>`：将本群以【公司名称】注册到市场，如果全群金币数小于两万则会注册失败。
...
`.股票回收`：回收退群用户，7天不在线用户，林汐不在的群内用户持有的股票。
